...
class Employee(Document):
    ...
    def update_user_permissions(self):
        frappe.permissions.add_user_permission(
            "Employee", self.name, self.user_id)
        frappe.permissions.set_user_permission_if_allowed(
            "Company", self.company, self.user_id)
    ...
    def validate_for_enabled_user_id(self):
        ...
        if enabled == 0:
            frappe.throw(_("User {0} is disabled").format(
    ...
    def validate_employee_leave_approver(self):
        for l in self.get("leave_approvers")[:]:
            if "Leave Approver" not in frappe.get_roles(l.leave_approver):
                frappe.get_doc("User", l.leave_approver).add_roles(
                    "Leave Approver")
    ...
    if date_of_birth:
        try:
            retirement_age = int(frappe.db.get_single_value(
                "HR Settings", "retirement_age") or 60)
...
def validate_employee_role(doc, method):
    ...
    if "Employee" in [d.role for d in doc.get("roles")]:
        if not frappe.db.get_value("Employee", {"user_id": doc.name}):
            frappe.msgprint(
                _("Please set User ID field in an Employee record to set Employee Role"))
...
def send_birthday_reminders():
    ...
    if birthdays:
        ...
        for e in birthdays:
            message = frappe.get_template("templates/emails/birthday_reminder.html").render({
